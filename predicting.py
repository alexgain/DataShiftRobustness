    ...
    for var_cur in range(len(var)):
        ...
        for i, (x,y) in enumerate(test_loader):
            ...
            if opt.prediction == 'alaa':
                ...
                grad_phi_S = torch.stack(torch.autograd.grad(loss_orig, [mean_S, var_S], retain_graph=True)).view(-1)               
            ...
            if opt.prediction == 'alaa2': #second order version
                ...
                grad_phi_S = torch.autograd.grad(loss_orig, phi_S, retain_graph=True)[0].view(-1)
                first_order = grad_phi_S.dot(phi_S)
                if first_order != first_order: #nan check
                    first_order = 0
                    
                #get hessian w.r.t. shift parameters at phi_S:
                grad_phi_S = torch.autograd.grad(loss_orig, phi_S, create_graph=True, retain_graph=True)[0]
